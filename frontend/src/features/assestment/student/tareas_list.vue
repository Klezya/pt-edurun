...
import { getTareasByCourseLmsId } from '../shared'
import type { Actividad } from '../shared'
...
onMounted(async () => {
  try {
    ...
    tareas.value = await getTareasByCourseLmsId(courseId) as Actividad[]
  } catch (error) {
...
function comenzar(tarea: Actividad) {
  router.push({ name: 'entorno-tarea', params: { id: tarea.id } })
}
</script>
    ...
    <div class="relative flex min-h-screen flex-col">
      ...
      <main class="flex flex-grow items-center justify-center px-4 py-8">
        <div class="mx-auto max-w-4xl w-full">
          ...
          <!-- Empty State -->
          <div v-else-if="tareas.length === 0" class="rounded-2xl border border-white/10 bg-slate-950/40 backdrop-blur-md shadow-2xl overflow-hidden">
